...
int SplitWayTable_TopDown(matrix2D &splitWayTable, int NtoSplit, int maxSubN){
	...
	if(maxSubN==0){
		...
		splitWayTable[NtoSplit][maxSubN]%=mod;
	}
	return splitWayTable[NtoSplit][maxSubN];
}
/*
...
int main(){
	int maxN = 1000;
	matrix2D splitWayTable(maxN, vector<int>(maxN, 0));
	/*
	...
	*/
	int n=5;
	int numWays = SplitWayTable_TopDown(splitWayTable, n-1, n-1);
	cout<<"There are "<<numWays<<" ways to split "<<n<<endl;
	while(numWays%mod!=0 && n<maxN){
		n++;
		numWays = SplitWayTable_TopDown(splitWayTable, n-1, n-1);
	}
	cout<<"There are "<<SplitWayTable_TopDown(splitWayTable, n-1, n-1)-1<<" ways to split "<<n<<endl;
